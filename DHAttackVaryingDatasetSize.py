...
def DistillModelUsingHardLabel_VaryDatasetSize(distill_dataset,original_model,dataset,num_epoch,dataFolderPath= './data/',modelFolderPath = './models/',classifierType = 'cnn', TargetOrShadow='Target'):
...
def init_args():
    ...
    parser.add_argument('--ref_dataset_size', type=int, default=1000)
...
if __name__ == '__main__':
    ...
    ref_dataset_size = args.ref_dataset_size  
    
    relabel_num = ref_dataset_size
    ...
    if(AttackMethod == "DHAttack"): 
        ...
        if distillFlag == True:
            distill_x, distill_y  = load_data_for_trainortest(dataPath + '/distillationData.npz')
            distill_x = distill_x.astype(np.float32)
            distill_y = distill_y.astype(np.int32)
            
            
            shadowTrain, shadowTrainLabel  = load_data_for_trainortest(dataPath + '/shadowTrain.npz')
            shadowTrain = shadowTrain.astype(np.float32)
            shadowTrainLabel = shadowTrainLabel.astype(np.int32) 

            shadowTest, shadowTestLabel  = load_data_for_trainortest(dataPath + '/shadowTest.npz') 
            shadowTest = shadowTest.astype(np.float32)
            shadowTestLabel = shadowTestLabel.astype(np.int32)


            Relabeled_x = np.concatenate([distill_x, shadowTrain, shadowTest], axis=0)
            Relabeled_y = np.concatenate([distill_y, shadowTrainLabel, shadowTestLabel], axis=0)
            
            relabel_num = relabel_num  
            

            dataIndex = list(range(len(Relabeled_y)))
            sampled_index = random.sample(dataIndex, relabel_num)  

            sampled_x = [Relabeled_x[i] for i in sampled_index]
            sampled_y = [Relabeled_y[i] for i in sampled_index]
            sampled_x = np.array(sampled_x)
            sampled_y = np.array(sampled_y)

            distill_dataset = (sampled_x, sampled_y)
            ...
            for i in range(distilledModelNum):
                ...
                print('use {} samples'.format(len(sampled_y)))
                distilledModel = DistillModelUsingHardLabel_VaryDatasetSize(distill_dataset, targetModel, dataset, num_epoch_for_distillation, classifierType =classifierType, modelFolderPath = './models/', TargetOrShadow='Target')
        ...
        for i in refModeleNames[:used_num]:
            ...
            refModeles.append(torch.load(modelsavepath + '/' + i)) #载入所有in_model。            
        ...
        boundary_distance_targetTrain_for_AllOut = np.vstack(boundary_distance_targetTrain_for_AllOut)  #(32,10000)
        boundary_distance_targetTrain_for_AllOut = np.swapaxes(boundary_distance_targetTrain_for_AllOut, 0, 1)  #(10000,32)
