...
def init_args():
    ...
    parser.add_argument('--localModelType', type=str, default='vgg')
...
if __name__ == '__main__':
    ...
    localModelType = args.localModelType
    ...
    if(AttackMethod == "DHAttack"): 
        ...
        my_loader1 = DataLoader(target_data, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)  
            
        noise_target_data = models.CIFARData(noisy_images_targetTrain, noisy_groundtruth_of_samples_targetTrain)
        my_loader2 = DataLoader(noise_target_data, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)  
        ...
        boundary_distance_targetTest_for_AllOut = np.vstack(boundary_distance_targetTest_for_AllOut)  
        boundary_distance_targetTest_for_AllOut = np.swapaxes(boundary_distance_targetTest_for_AllOut, 0, 1) 
        ...
        print(disturb_num)
        print(used_num)
        np.savez(resultDataPath + '{}_{}_{}_disturb_num{}_used_num{}.npz'.format(AttackMethod,dataset,classifierType,disturb_num,used_num), answers, prediction)  
